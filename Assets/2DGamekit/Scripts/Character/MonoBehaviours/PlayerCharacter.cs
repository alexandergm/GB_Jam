...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        public float velocityFlyRange = 0.5f;
        public float maxFlyRange = 2;
        ...
        public float timerToHelicopter;
        ...
        protected Coroutine m_HelicopterCoroutine;
        ...
        protected bool superPower = false;
        protected bool isSuperPowerActive = false;
        protected bool isSuperDamage = false;
        ...
        void Update()
        {
            if (isSuperPowerActive && CheckForGrounded() && superPower)
            {
                StartCoroutine(GroundDamageHelicopter());
            }  
            if(PlayerInput.Instance.Helicopter.Up)
            {
                DisableMeleeAttack();
            }
            if (CheckForGrounded())      //      
                ...
                timerToHelicopter += Time.deltaTime;
        ...
        {
            if (!PlayerInput.Instance.Jump.Held && m_MoveVector.y > 0.0f && !PlayerInput.Instance.Helicopter.Held)
            {
        ...
        public void AirborneVerticalMovement()
        {
            ...
            if (!PlayerInput.Instance.Helicopter.Down)
                m_MoveVector.y -= gravity * Time.deltaTime;
        }
        ...
        public bool CheckForHoldingGun()
        {
            ...
            if (PlayerInput.Instance.RangedAttack.Held && !PlayerInput.Instance.Helicopter.Held)
            {
                holdingGun = true;
        ...
        {
            if (PlayerInput.Instance.RangedAttack.Held && m_Animator.GetBool(m_HashHoldingGunPara) && !PlayerInput.Instance.Helicopter.Held)
            {
        ...
        {
            if (PlayerInput.Instance.Helicopter.Held && !m_CharacterController2D.IsGrounded || PlayerInput.Instance.Helicopter.Held && PlayerInput.Instance.Jump.Down)
            {
                if (m_HelicopterCoroutine == null)
                    m_HelicopterCoroutine = StartCoroutine(FlyHelicopter());
            }
            if (PlayerInput.Instance.Helicopter.Up && m_HelicopterCoroutine != null || m_CharacterController2D.IsGrounded && m_HelicopterCoroutine != null)
            {
                StopCoroutine(m_HelicopterCoroutine);                
                ChangeDamageOffset(1.5f, 1);
                helicopter.SetActive(false);
                m_HelicopterCoroutine = null;
            }
        }
        protected IEnumerator FlyHelicopter()
        {
            ChangeDamageOffset(0, 2.5f);
            helicopter.SetActive(true);
            yield return new WaitForSeconds(0.05f);
            isSuperPowerActive = true;
            while (PlayerInput.Instance.Helicopter.Held)
            {
                if (m_MoveVector.y <= maxFlyRange && timerToHelicopter < 4f)
                {                    
                    IncrementVerticalMovement(velocityFlyRange);
                }
                yield return null;
            }

        }
        protected IEnumerator GroundDamageHelicopter()
        {            
            ChangeDamageOffset(0, 0.5f, 6.5f);
            ...
            ChangeDamageOffset(1.5f, 1, 2.5f);
        }
        private void ChangeDamageOffset(float x, float y)
        {
            meleeDamager.offset.x = x;
            meleeDamager.offset.y = y;
        }
        private void ChangeDamageOffset(float x, float y, float size)
        {
            meleeDamager.offset.x = x;
            meleeDamager.offset.y = y;
            meleeDamager.size.x = size;
        }        
        ...
        public bool CheckForHelicopter()
        {
            return PlayerInput.Instance.Helicopter.Held && !CheckForGrounded();
        }
        public bool CheckForSuperPower()
        {
            return isSuperDamage;
        }

        public void MeleeAttack()
        {
            if (!PlayerInput.Instance.Helicopter.Held)
